...
import { listMySearches, toggleSavedSearch, deleteSavedSearch, type SavedSearch } from '../../actions/saved-searches';
import { useAuth } from '../../../lib/auth/auth-context';
import { formatShortDate } from '../../../lib/format';

/**
 * Mes annonces = alias de « Mes avis de recherche » avec le label utilisé
 * dans la navigation de l'image de référence.
 */
export default function AnnoncesPage() {
  ...
  async function handleToggle(search: SavedSearch) {
    ...
    if (!result.ok) {
      setItems((prev) => prev.map((s) => (s.id === search.id ? { ...s, is_active: search.is_active } : s)));
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <p role="alert" className="text-body text-danger-700">{error}</p>
      ) : items.length === 0 ? (
        <EmptyState
